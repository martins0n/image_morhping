...
fn morphy<'a>(
    ...
    for (x, y, pixel) in target.enumerate_pixels_mut() {
        ...
        for lp in line_pairs.iter() {
            ...
            let v = (((X - lp.t_line.p) * (lp.t_line.q - lp.t_line.p).perpendicular()) as f32)
                / (lp.t_line.q.dist2(lp.t_line.p) as f32).sqrt();
            let X_new = lp.s_line.p
                + (lp.s_line.q - lp.s_line.p).mul(u as f32)
                + (lp.s_line.q - lp.s_line.p)
                    .perpendicular()
                    .mul(v)
                    .mul(1.0 / (lp.s_line.q.dist2(lp.s_line.p) as f32).sqrt());
            let D = X_new - X;
            let weight = 1.0;
...
fn main() {
    ...
    let mut lp = vec![LinePair {
        t_line: Line {
            p: Vector { x: 100, y: 100 },
            q: Vector { x: 100, y: 200 },
        },
        s_line: Line {
            p: Vector { x: 100, y: 200 },
            q: Vector { x: 100, y: 200 },
        },
    }];
